...
(function () {
  ...
  const equivalentPlugins = [
    pluginName,
  ];
  const hasEquivalentPlugin = $plugins.some((item) => equivalentPlugins.includes(item.name));
  if (hasEquivalentPlugin) return;

  var getCommandName = function (command) {
    return (command || '').toUpperCase();
  };

  var getParamBoolean = function (paramNames, defaultValue) {
  ...
  var getParamArrayNumber = function (paramNames) {
    ...
    return JSON.parse(JSON.stringify(value, paramReplacer));
  };
  ...
  // パラメータの取得と整形
  //=============================================================================
  var paramDirection8Move = getParamBoolean(['Direction8Move', '8方向移動'], true);
  var paramEventThrough = getParamBoolean(['EventThrough', 'イベントすり抜け'], false);
  var paramDisableForcing = getParamBoolean(['DisableForcing', '強制中無効'], true);
  var paramAvoidCorner = getParamBoolean(['AvoidCorner', '角回避'], true);
  var paramDiagonalSlow = getParamBoolean(['DiagonalSlow', '斜め移動中減速'], false);
  var paramTriggerExpansion = getParamBoolean(['TriggerExpansion', 'トリガー拡大'], true);
  var paramAdjustmentRealStep = getParamBoolean(['AdjustmentRealStep', '実歩数調整'], false);
  ...
  var paramMultiStartDisable = getParamBoolean(['MultiStartDisable', 'イベント複数起動防止'], true);
  var paramEventOverlap = getParamBoolean(['EventOverlap', 'イベント位置重複OK'], false);
