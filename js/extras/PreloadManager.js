 ...
 */
/*:ja
 * @plugindesc マップ/シーン切り替え時、またブート時に素材の事前ロードを行います。
 *
 * @author Tor Damian Design / Galenmereth
 *
 * @param Preload System SFX
 * @desc ブート時に、データベース上で指定された全てのSFXをプリロードしたい場合は、trueにしてください。
 * @default false
 *
 * @param Print Debug to Console
 * @desc コンソール(F8)でデバッグ情報を見たい場合は、trueにしてください。
 * @default false
 *
 * @help =~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~
 * Introduction / Table of contents
 * =~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~
 * TDDP PreloadManagerは、画像・音楽・サウンドエフェクトのような素材を、
 * 事前にロードさせることができるプラグインです。
 * タイトル前のゲームブート時や、マップ切り替わりのロード時に、
 * 事前ローディングを可能にします。
 *
 * デフォルト設定時には、素材のストリームは要求後に行われますが、
 * このプラグインを利用することで、高帯域を使用し、
 * スムーズなゲームプレイを提供することが可能になります。 
 * 
 * このプラグインは、プレイヤーのアクションがトリガーとなるか否かに関わらず、
 * マップ上のすべてのイベントページで、全素材をロードします。
 * ページの条件分岐がプレイヤーのアクションに依存しないため、
 * 必ず正しいファイルがロードされるように注意してください。
 * 
 * アップデートと簡易使用方法については、
 ...
 * 1. Installation
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * プラグインの「.js」ファイルを、プロジェクト内のプラグインフォルダに
 * ドラッグ&ドロップし、エディター内で有効にしてください。
 *
 * 最大の互換性を得るため、このプラグインはトップに置くようにしてください。
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * 2. Advanced Configuration
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * プラグインの「.js」ファイルを開くと、
 * アドバンスドコンフィグレーションを行うことができます。
 * BOOT PRELOAD CONFIG -- ADVANCED USERSを用いて、
 * ブート時にプリロードされる素材を手動で指定することができます。
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 * 3. Terms & Conditions
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* =============================================================================
*
* Tip: ファイル名は省略することができます。
* 拡張子はMVの特性に合わせ、「.png」とみなされます。
*
* -----------------------------------------------------------------------------
...
*   title2
*
* 上記のフォルダ内に、サブフォルダを作成している場合は、
* ファイル名にそれを付け足してください。
* 例えば「arrow」という画像が「cursors」というサブフォルダの中に入っている場合
* 下記のように指定してください。
* 
* system: [
*   "balloon",
